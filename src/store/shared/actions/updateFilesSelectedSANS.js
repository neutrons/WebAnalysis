...
export default async ({ state, dispatch, commit, rootGetters }, payload) => {
  ...
  const [storedData, nonStoredFiles] = rootGetters['SANS/getStoredData'](filelist);

  // Next Get File References, either url to fetch or blob to file read
  const [urls, blobs] = rootGetters['SANS/getURLs'](nonStoredFiles);
  ...
  // Next Store Data
  commit('SANS/storeData', parsedData, { root: true });
